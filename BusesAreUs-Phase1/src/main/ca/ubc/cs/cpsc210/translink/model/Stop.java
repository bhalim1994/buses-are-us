...
public class Stop implements Iterable<Arrival> {
    ...
    public void addRoute(Route route) {
        if (!routes.contains(route)) {
            routes.add(route); //Need to add to list to keep track of routes added when you call getRoutes method
            route.addStop(this);
        }
    }
    ...
    public void addBus(Bus bus) throws RouteException {
        if (!routes.contains(bus.getRoute())) {
            throw new RouteException("Bus not on a route on which this stop lies!");
        } else {
            buses.add(bus);
        }
    }
